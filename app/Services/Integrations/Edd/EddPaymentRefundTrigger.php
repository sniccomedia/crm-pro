...
class EddPaymentRefundTrigger extends BaseTrigger
{
    public function __construct()
    {
        $this->triggerName = 'edd_fc_order_refunded_simulation';
        $this->priority = 10;
        $this->actionArgNum = 3;
        parent::__construct();
    ...
    public function getTrigger()
    {
        return [
            ...
            'label'       => __('Payment Refunded', 'fluentcampaign-pro'),
            'description' => __('This Funnel will start once payment refunded for an order', 'fluentcampaign-pro')
        ];
    }
    ...
    {
        return [
            'title'     => __('Payment Refunded in EDD', 'fluentcampaign-pro'),
            'sub_title' => __('This Funnel will start once an order get marked as refunded', 'fluentcampaign-pro'),
            'fields'    => [
                'subscription_status'      => [
    ...
    public function getConditionFields($funnel)
    {
        return [
            ...
            'product_ids'        => [
                ...
                'inline_help' => __('Keep it blank to run to any product', 'fluentcampaign-pro')
            ],
            'product_categories' => [
                'type'        => 'multi-select',
    ...
    public function handle($funnel, $originalArgs)
    {
        ...
        if ($newStatus != 'refunded' || $newStatus == $oldStatus) {
            return;
        }
